// scroll based animation utility class
class ScrollBasedAnimation {
  ...
  create() {
    for (let i of this.options) {
      ...
      for (let j in i.values) {
        e.style[j] = i.values[j].unit
          ? i.values[j].from + i.values[j].unit
          : i.values[j].from;
      }
    }
  ...
  scrollHandler(ev) {
    ...
    for (let i of this.options) {
      ...
      for (let j in i.values) {
        let styleValue =
          i.values[j].from + (i.values[j].to - i.values[j].from) * p;
        e.style[j] = i.values[j].unit
          ? styleValue + i.values[j].unit
          : styleValue;
      }
    }
  }
